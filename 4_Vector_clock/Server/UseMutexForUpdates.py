            ...
            try: 
                ...
                for idx, proxy in enumerate(self.proxies):
                    if idx == self.myID:
                        match command:
                            case "PUT":
                                await self.messageBoard.put(nextRequest[1])
                            case "MODIFY":
                                await self.messageBoard.modify(nextRequest[1], nextRequest[2])
                            case "DELETE":
                                await self.messageBoard.delete(nextRequest[1])
                            case "DELETEALL":
                                await self.messageBoard.deleteAll()
                    else:
                        match command:
                            case "PUT":
