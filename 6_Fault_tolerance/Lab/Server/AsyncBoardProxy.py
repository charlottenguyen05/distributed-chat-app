...
class storage:
    def __init__(self, port, senderId, logicalClock=None, websocketconnect=client.connect):
        """
        ...
        self.websocketconnect = websocketconnect
        self.ws = None
    ...
    async def doOperation(self, request):
        ...
        async with self.lock:
            ...
            if self.ws is None:
                try:
                    self.ws = await self.websocketconnect(f"ws://localhost:{self.port}/")
    ...
    async def put(self, message, seqNb=None):
        ...
        if response_object["status"] == "ERROR":
            raise ConnectionError(f"PUT failed: {response_object['error']}")
    ...
    async def modify(self, index, message,seqNb=None):
        ...
        response_object = await self.doOperation(request)
        if response_object["status"] == "ERROR":
            raise ConnectionError(f"MODIFY failed: {response_object['error']}")
    ...
    async def delete(self, index,seqNb=None):
        ...
        response_object = await self.doOperation(request)
        if response_object["status"] == "ERROR":
            raise ConnectionError(f"DELETE failed: {response_object['error']}")
    ...
    async def deleteAll(self,seqNb=None):
        ...
        response_object = await self.doOperation(request)
        if response_object["status"] == "ERROR":
            raise ConnectionError(f"DELETEALL failed: {response_object['error']}")
